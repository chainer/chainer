...
def _is_cupy_array(array):
    return chainer.backends.get_array_module(array) is not numpy
...
class MpiCommunicatorBase(communicator_base.CommunicatorBase):
    ...
    def alltoall(self, xs):
        ...
        xp = chainer.backends.get_array_module(*xs)
    ...
    def send(self, data, dest, tag):
        ...
        for array in data:
            if chainer.backends.get_array_module(array) is not numpy:
                chainer.cuda.Stream.null.synchronize()
    ...
    def gather(self, x, root=0):
        ...
        if is_master:
            ...
            xp = chainer.backends.get_array_module(x)
