...
class IpcMemoryHandle(object):
    ...
    Args:
        array (cupy.ndarray): cupy ndarray to be shared among
                              multiple processes

    Attributes:
        mh_data (list): data of memory handle to be created by
                        cudaIpcGetMemHandle()

        dtype (numpy.dtype): Dtype object of element type
        size (int): Number of elements
        device_id (int): Device ID
        devptr (int): Pointer to device memory which is initiated by
                      another process
    """
