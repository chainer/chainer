...
Primitive links that wrap functions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
A link that does not depend on other links is called *a primitive link*.
We have already seen how to define a link that wraps other links (by inheriting :class:`DictLink` class).
Here we study how to define a primitive link.
...
.. testcode::
   ...
   class EltwiseParamProduct(Link):
       def __init__(self, shape):
           ...
           self.params['w'] = Variable(np.random.randn(*shape).astype(np.float32))
...
Another example: assume we want to define a simple linear layer.
   ...
   class Linear(Link):
       def __init__(self, in_size, out_size):
           ...
           self.params['W'] = Variable(
               (np.random.randn(out_size, in_size) / math.sqrt(in_size)).astype(np.float32))
           self.params['b'] = Variable(np.random.zeros(out_size, dtype=np.float32))

       def __call__(self, x):
           return linear(x, self.params['W'], self.params['b'])
