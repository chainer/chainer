...
In this tutorial, w generate images with **generative adversarial networks (GAN)**.
It is a kind of generative models with deep neural networks, and often applied to
image generation. GAN-based models are also used in
...
so they will build more sophisticated counterfeit banknotes. As the police improve their skill further,
...
discriminator :math:`D({\bf s})` and the generator :math:`G({\bf z})` [4].
