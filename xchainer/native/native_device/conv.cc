...
Array NativeDevice::Conv(
    ...
    Array col = internal::Im2Col(x.AsGradStopped(), kernel_size, stride, pad, cover_all, 0);

    // Compute the tensor dot product of col and w, reducing (channel, k_1, k_2, ..., k_n).
    Axes axes;
    axes.resize(ndim + 1);
    std::iota(axes.begin(), axes.end(), 1);
    Array y = TensorDot(col, w.AsGradStopped(), axes, axes);  // (batch_size, out_1, out_2, ..., out_n, out_channel)
...
Array NativeDevice::ConvGradWeight(
    ...
    return TensorDot(gy.AsGradStopped(), col, out_axes, col_axes).AsType(w_dtype, false);
}
