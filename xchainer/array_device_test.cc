...
TEST_F(ArrayDeviceTest, FromContiguousHostMemory) {
    Shape shape({2, 3});
    ...
    CheckDeviceFallback([&]() { return Array::FromContiguousHostMemory(shape, dtype, data); });
    CheckDeviceExplicit([&](Device& device) { return Array::FromContiguousHostMemory(shape, dtype, data, device); });
}
