...
OpNode::OpNode(
    ...
    : name_{std::move(name)}, graph_id_{prev_array_nodes.front()->graph_id()}, prev_array_props_{std::move(prev_array_props)} {
    assert(prev_array_props_.size() == prev_array_nodes.size());
