...
template <typename T>
void CheckArrayRepr(
    const std::string& expected, const std::vector<T>& data_vec, Shape shape, Device& device, const std::vector<GraphId>& graph_ids = {}) {
...
TEST(ArrayReprTest, AllDtypesOnNativeBackend) {
    ...
    // Two graphs
    CheckArrayRepr<int32_t>("array([1], shape=(1,), dtype=int32, device='native:0', graph_ids=['graph_1', 'graph_2'])",
                            {1},
                            Shape({1}),
                            device,
                            {"graph_1", "graph_2"});
