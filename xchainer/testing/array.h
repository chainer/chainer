...
public:
    ...
    ArrayBuilder& WithData(InputIter first, InputIter last) {
        ...
        create_array_ = [data](const ArrayBuilder& builder) -> Array {
            ...
            return internal::FromHostMemory(shape, dtype, std::move(ptr), std::move(strides), builder.device_);
        };
        return *this;
    }
...
Array BuildArray(const Shape& shape, const std::vector<T>& data) {
    ...
    return Array::FromContiguousHostMemory(shape, TypeToDtype<T>, std::move(a));
}
