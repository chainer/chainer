...
protected:
    template <typename T>
    Array MakeArray(const Shape& shape, const T* data, bool requires_grad = false, const std::string& graph_name = "") const {
        int64_t size = shape.total_size();
        auto a = std::make_unique<T[]>(size);
        std::copy(data, data + size, a.get());
        //Array arr = Array::FromBuffer(shape, TypeToDtype<T>, std::move(a)).GetNode("", true);
        Array arr = Array::FromBuffer(shape, TypeToDtype<T>, std::move(a));
    ...
    void CheckBaseBackwardComputation(bool expect_correct, Fprop fprop, const Arrays& inputs, const Arrays& grad_outputs, const Arrays& eps,
                                      double atol, double rtol, const std::string& graph_name) {
        if (!expect_correct && std::any_of(inputs.begin(), inputs.end(), [graph_name](const Array& input) { return input.requires_grad(graph_name); })) {
            // Catch the gtest failure expected to be generated by CheckBackwardComputation but without failing this test
            EXPECT_NONFATAL_FAILURE(CheckBackwardComputation(fprop, inputs, grad_outputs, eps, atol, rtol, graph_name), "Backward check failure");
        } else {
            // We cannot expect any failures in case none of the input std::vector<Array> require gradients
            CheckBackwardComputation(fprop, inputs, grad_outputs, eps, atol, rtol, graph_name);
        }
    }
};
...
protected:
    ...
    void CheckBackwardComputation(bool expect_correct, Fprop fprop, const Shape& shape, const T* input_data, const T* grad_output_data,
                                  const T* eps_data, double atol, double rtol, const std::string& graph_name) {
        Arrays inputs{MakeArray(shape, input_data, true, graph_name)};
        Arrays grad_outputs{MakeArray(shape, grad_output_data)};
        Arrays eps{MakeArray(shape, eps_data)};
        CheckBaseBackwardComputation(expect_correct, fprop, inputs, grad_outputs, eps, atol, rtol, graph_name);
    }
...
protected:
    ...
    void CheckBackwardComputation(bool expect_correct, Fprop fprop, const Shape& shape, const T* input_data1, const T* input_data2,
                                  const T* grad_output_data, const T* eps_data1, const T* eps_data2, double atol, double rtol, const std::string& graph_name) {
        Arrays inputs{MakeArray(shape, input_data1, true, graph_name), MakeArray(shape, input_data2, true, graph_name)};
        Arrays grad_outputs{MakeArray(shape, grad_output_data)};
...
TEST_P(CheckBackwardBinaryTest, CorrectBackward) {
    ...
    CheckBackwardComputation(true, fprop, {1, 3}, input_data1, input_data2, grad_output_data, eps_data1, eps_data2, 1e-5, 1e-4, "graph_2");
}
