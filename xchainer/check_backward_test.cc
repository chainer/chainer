...
protected:
    ...
    void CheckCheckBackward(
        ...
        if (requires_grad_) {
            inputs[0].RequireGrad(backprop_scope.graph_id());
        }
        ...
        bool is_none_of_grad_required = std::none_of(inputs.begin(), inputs.end(), [&backprop_scope](const Array& input) {
            return input.IsGradRequired(backprop_scope.graph_id());
        });
        if (expect_correct || is_none_of_grad_required) {
            ...
            CheckBackward(fprop, inputs, grad_outputs, eps, atol, rtol, backprop_scope.graph_id());
        } else {
            // Catch the gtest failure expected to be generated by CheckBackward but without failing this test
            EXPECT_THROW(CheckBackward(fprop, inputs, grad_outputs, eps, atol, rtol, backprop_scope.graph_id()), GradientCheckError);
        }
    }
...
protected:
    ...
    void CheckCheckDoubleBackward(
        ...
        for (auto& input : inputs) {
            input.RequireGrad(backprop_scope.graph_id());
        }
        for (auto& grad_output : grad_outputs) {
            grad_output.RequireGrad(backprop_scope.graph_id());
        }

        // A failure occurs if backward computation and numerical gradients have differences
        CheckDoubleBackwardComputation(fprop, inputs, grad_outputs, grad_grad_inputs, eps, atol, rtol, backprop_scope.graph_id());
    }
