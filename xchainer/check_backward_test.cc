...
protected:
    ...
    void CheckCheckBackward(
        ...
        GraphScope graph_scope{graph_name};

        if (requires_grad_) {
            inputs[0].RequireGrad(graph_scope.graph_id());
        }
        ...
        bool is_none_of_grad_required = std::none_of(
                inputs.begin(), inputs.end(), [&graph_scope](const Array& input) { return input.IsGradRequired(graph_scope.graph_id()); });
        if (expect_correct || is_none_of_grad_required) {
            ...
            CheckBackward(fprop, inputs, grad_outputs, eps, atol, rtol, graph_scope.graph_id());
        } else {
            // Catch the gtest failure expected to be generated by CheckBackward but without failing this test
            EXPECT_THROW(CheckBackward(fprop, inputs, grad_outputs, eps, atol, rtol, graph_scope.graph_id()), GradientCheckError);
        }
    }
...
protected:
    ...
    void CheckCheckDoubleBackward(
        ...
        for (auto& input : inputs) {
            input.RequireGrad(graph_scope.graph_id());
        }
        for (auto& grad_output : grad_outputs) {
            grad_output.RequireGrad(graph_scope.graph_id());
        }

        // A failure occurs if backward computation and numerical gradients have differences
        CheckDoubleBackwardComputation(fprop, inputs, grad_outputs, grad_grad_inputs, eps, atol, rtol, graph_scope.graph_id());
    }
