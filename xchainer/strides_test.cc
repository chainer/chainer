...
TEST(StridesTest, Ctor) {
    {
        ...
        CheckSpanEqual({2, 3, 4}, strides.span());
    }
    {
...
TEST(StridesTest, Subscript) {
    const Strides strides = {2, 3, 4};
    EXPECT_EQ(2, strides[0]);
    EXPECT_EQ(3, strides[1]);
...
TEST(StridesTest, Compare) {
    {
        const Strides strides = {2, 3, 4};
        const Strides strides2 = {2, 3, 4};
        EXPECT_TRUE(strides == strides2);
    }
    {
        const Strides strides = {2, 3, 4};
        const Strides strides2 = {2, 3};
        EXPECT_TRUE(strides != strides2);
    }
    {
        const Strides strides = {2, 3, 4};
        const Strides strides2 = {1, 2, 3};
        EXPECT_TRUE(strides != strides2);
    }
}
...
TEST(StridesTest, Iterator) {
    const Strides strides = {2, 3, 4};
    CheckSpanEqual({2, 3, 4}, gsl::make_span(strides.begin(), strides.end()));
    CheckSpanEqual({4, 3, 2}, gsl::make_span(std::vector<int64_t>{strides.rbegin(), strides.rend()}));
}
...
TEST(StridesTest, ToString) {
    ...
    }
    {
        ...
        EXPECT_EQ(strides.ToString(), "(1,)");
    }
    {
        const Strides strides = {2, 3, 4};
        EXPECT_EQ(strides.ToString(), "(2, 3, 4)");
    }
}
