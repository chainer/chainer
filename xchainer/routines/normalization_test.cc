...
TEST_P(NormalizationTest, BatchNormalization) {
    ...
    Shape reduced_shape{1, 4, 4};
    Scalar eps{2e-5f};
    Scalar decay{0.9f};

    Array a = testing::BuildArray(x_shape).WithLinearData<T>();
    Array gamma = testing::BuildArray(reduced_shape).WithLinearData<T>();
    Array beta = testing::BuildArray(reduced_shape).WithLinearData<T>();
    Array running_mean = testing::BuildArray(reduced_shape).WithLinearData<T>();
    Array running_var = testing::BuildArray(reduced_shape).WithLinearData<T>();
    ...
    Array e_running_mean = testing::BuildArray(reduced_shape)
                                   .WithData<float>({1.6,
                                                     2.6,
                                                     3.6,
                                                     4.6,
    ...
    Array e_running_var = testing::BuildArray(reduced_shape)
                                  .WithData<float>({25.600002,
                                                    26.500002,
                                                    27.400002,
                                                    28.300003,
                                                    29.200003,
...
TEST_P(NormalizationTest, BatchNormalizationWithAxis) {
    ...
    Array e_running_mean = testing::BuildArray(reduced_shape).WithData<float>({0.35, 2.05, 3.75});  // Computed with Chainer.

    Array e_running_var = testing::BuildArray(reduced_shape).WithData<float>({0.6, 1.5, 2.4});  // Computed with Chainer.
