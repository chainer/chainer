...
public:
    ...
    OpNode(std::string name, int64_t rank, std::vector<std::shared_ptr<ArrayNode>> next_nodes,
           std::vector<std::function<Array(const GraphId&, const Array&)>> backward_functions)
        : name_(std::move(name)), rank_(rank), next_nodes_(std::move(next_nodes)), backward_functions_(std::move(backward_functions)) {}

    void RegisterNextNode(std::shared_ptr<ArrayNode> next_node, std::function<Array(const GraphId&, const Array&)> backward_function) {
...
private:
    ...
    std::vector<std::function<Array(const GraphId&, const Array&)>> backward_functions_;
};
