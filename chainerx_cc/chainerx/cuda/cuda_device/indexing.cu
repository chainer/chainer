...
template <typename T, typename TIndex>
__global__ void CumsumCudaKernel(
        IndexableArray<const T> a_iarray,
        IndexableArray<T> out_iarray,
        Indexer<> a_indexer,
        Indexer<> out_indexer,
    ...
    for (auto it = out_indexer.It(blockIdx.x * blockDim.x + threadIdx.x, blockDim.x * gridDim.x); it; ++it) {
        ...
        out_iarray[it] = temp[pout * n + it.raw_index()];
    }
}

template <typename TIndex>
void CumsumImpl(Device& device, const Array& a, int8_t axis, const Array& out) {
    static_assert(std::is_same<TIndex, int64_t>::value || std::is_same<TIndex, int32_t>::value, "");
    ...
    VisitDtype(out.dtype(), [&a, axis, &out](auto pt) {
        ...
        std::lock_guard<std::mutex> lock{*cuda_internal::g_mutex};
        static const int kMaxBlockSize = CudaOccupancyMaxPotentialBlockSize(&CumsumCudaKernel<T, TIndex>).block_size;
        int64_t total_size = out_indexer.total_size();
        int64_t grid_size = (total_size + kMaxBlockSize - 1) / kMaxBlockSize;
        int64_t block_size = std::min<TIndex>(total_size, kMaxBlockSize);

        CumsumCudaKernel<<<grid_size, block_size>>>(
                a_iarray, out_iarray, a_indexer, out_indexer, indices_indexer, common_total_size, axis_dim);
    });
}
...
public:
    void Call(const Array& a, int8_t axis, const Array& out) override {
        ...
        CumsumImpl<int64_t>(device, a, axis, out);
    }
};
