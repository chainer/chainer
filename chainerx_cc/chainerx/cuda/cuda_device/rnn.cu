    ...
    for (int layer = 0 ; layer < n_layers; layer++) {
        for (int8_t di = 0; di < num_directions; di++) {
            for (uint lin_layer_id = 0; lin_layer_id < ws[0].size(); lin_layer_id++) {
                ...
                initGPUData(m_offset, filterDimA[0] * filterDimA[1] * filterDimA[2],
                    reinterpret_cast<float*>(internal::GetRawOffsetData(ws[index][lin_layer_id])));
                ...
                initGPUData(b_offset, filterDimA[0] * filterDimA[1] * filterDimA[2],
                    reinterpret_cast<float*>(internal::GetRawOffsetData(bs[index][lin_layer_id])));
    ...
    for (int64_t layer = 0 ; layer < n_layers; layer++) {
        for (int8_t di = 0; di < num_directions; di++) {
            for (uint lin_layer_id = 0; lin_layer_id < ws[0].size(); lin_layer_id++) {
                ...
                &nbDims,
                filterDimA);
                initGPUData(reinterpret_cast<float*>(internal::GetRawOffsetData(m)),
                    filterDimA[0] * filterDimA[1] * filterDimA[2], m_offset);
                cudnnDestroyFilterDescriptor(linlayermatdesc);
                ret.push_back(m);
                cudnnFilterDescriptor_t linlayerbiasdesc;
                cudnnCreateFilterDescriptor(&linlayerbiasdesc);
                ...
                cudnnGetFilterNdDescriptor(
                                        ...
                                        &nbDims,
                                        filterDimA);
                initGPUData(reinterpret_cast<float*>(internal::GetRawOffsetData(b)),
                    filterDimA[0] * filterDimA[1] * filterDimA[2], b_offset);
                cudnnDestroyFilterDescriptor(linlayerbiasdesc);
                ret.push_back(b);
            }
        }
...
public:
    std::tuple<std::vector<std::vector<Array>>, std::unique_ptr<chainerx::RnnGradState>> Call(
        ...
        for (uint i = 0; i < xs.size(); i++) {
            ...
            ys.push_back(AsContiguous(Zeros({xs_cont[i].shape()[0], num_directions * hidden_dim},
                xs_cont[i].dtype(), xs_cont[i].device())));
