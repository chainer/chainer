...
template <int8_t Ndim, typename Op, typename... Ts>
__global__ void ElementwiseKernelWithIndex(Op op, Indexer<Ndim> indexer, IndexableArray<Ts, Ndim>... args) {
    for (auto it = indexer.It(blockIdx.x * blockDim.x + threadIdx.x, blockDim.x * gridDim.x); it; ++it) {
        const int64_t* index = it.index();
        op(index, cuda_internal::StorageToDataType<Ts>(args[it])...);
    }
}
...
void LaunchElementwiseKernelWithIndex(Op&& op, const Shape& shape, const Arrays&... args) {
    ...
    static const int kMaxBlockSize = CudaOccupancyMaxPotentialBlockSize(&ElementwiseKernelWithIndex<Ndim, Op, Ts...>).block_size;
