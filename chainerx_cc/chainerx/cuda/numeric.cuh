...
__device__ inline float Log1p(float x) { return std::log1p(x); }

__device__ inline cuda::Float16 Log1p(cuda::Float16 x) { return cuda::Float16{std::log1p(static_cast<float>(x))}; }
