...
Array Prod(const Array& a, const OptionalAxes& axis, bool keepdims) {
    ...
    {
        NoBackpropModeScope scope{};
        a.device().backend().CallKernel<ProdKernel>(a, sorted_axis, out);
    }

    BackwardBuilder bb{"prod", a, out};
    if (BackwardBuilder::Target bt = bb.CreateTarget(0)) {
        bt.Define([sorted_axis, a_tok = bb.RetainInput(0), out_tok = bb.RetainOutput(0), in_shape = a.shape(), keepdims](
