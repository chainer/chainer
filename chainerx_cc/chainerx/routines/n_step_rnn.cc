...
std::vector<Array> _lstm(const Array& x, const Array& h, const nonstd::optional<Array>& c, const std::vector<Array>& ws, const std::vector<Array>& bs) {
...
template <typename Impl>
std::vector<std::vector<Array>> n_step_rnn_impl(Impl&& impl, int64_t n_layers, Array hx, nonstd::optional<Array> cx, const std::vector<std::vector<Array>>& ws, 
	const std::vector<std::vector<Array>>& bs, std::vector<Array>& xs, const int8_t use_bidirection) {
	...
	for(int64_t layer = 0; layer < n_layers; layer++) {
		xs = xs_next;
		idx = direction * layer;
		std::vector<std::vector<Array>> one_directional_out_fw = _one_directional_loop(impl, xs, hx_list[idx], cx_list[idx], ws[idx], bs[idx]);
