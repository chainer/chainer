...
template <int8_t Ndim, typename Op, typename... Ts>
void ElementwiseKernelWithIndex(Op op, const Indexer<Ndim>& indexer, const IndexableArray<Ts, Ndim>&... args) {
    for (auto it = indexer.It(0, 1); it; ++it) {
        const int64_t* index = it.index();
        op(index, native_internal::StorageToDataType<Ts>(args[it])...);
    }
}
