...
void Col2ImImpl(const Array& col, const Array& out, const StackVector<int64_t, kMaxNdim>& stride, const StackVector<int64_t, kMaxNdim>& dilate, const Indexer<2>& batch_channel_indexer) {
        ...
        for (it_kernel.Restart(); it_kernel; ++it_kernel) {
            ...
            for (it_in_image_dims.Restart(); it_in_image_dims; ++it_in_image_dims) {
                for (int8_t i = 0; i < kKernelNdim; ++i) {
                    out_image_index.index()[i] = it_in_image_dims.index()[i] * stride[i] + it_kernel.index()[i] * dilate[i];
                }
...
Array Col2Im(
    ...
    VisitDtype(col.dtype(), [&](auto pt) {
        ...
        switch (ndim) {
            case 0:
                Col2ImImpl<T, 0>(col, padded_out, stride, dilate, batch_channel_indexer);
                break;
            case 1:
                Col2ImImpl<T, 1>(col, padded_out, stride, dilate, batch_channel_indexer);
                break;
            case 2:
                Col2ImImpl<T, 2>(col, padded_out, stride, dilate, batch_channel_indexer);
                break;
            case 3:
                Col2ImImpl<T, 3>(col, padded_out, stride, dilate, batch_channel_indexer);
                break;
            case 4:
                Col2ImImpl<T, 4>(col, padded_out, stride, dilate, batch_channel_indexer);
                break;
            default:
