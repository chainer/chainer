...
public:
    Array Call(
        ...
        if (groups > 1) {
            ...
            std::vector<chainerx::Array> outputs(groups);
            for (int i = 0; i < groups; ++i) {
                auto sub_bias = b.has_value() ? absl::optional<chainerx::Array>(biases[i]) : absl::nullopt;
                outputs[i] = Call(inputs[i], weights[i], sub_bias, stride, pad, 1, cover_all, out_dtype, absl::nullopt);
            }
...
public:
    Array Call(
        ...
        if (groups > 1) {
            std::vector<chainerx::Array> xs = chainerx::Split(x, groups, 1);
            std::vector<chainerx::Array> gys = chainerx::Split(gy, groups, 1);
            std::vector<chainerx::Array> gws(groups);
            for (int g = 0; g < groups; ++g) {
                gws[g] = Call(w_dtype, w_shape, xs[g], gys[g], stride, pad, 1, cover_all, absl::nullopt);
            }
...
public:
    Array Call(
        ...
        if (groups > 1) {
            ...
            for (int i = 0; i < groups; ++i) {
                auto sub_bias = b.has_value() ? absl::optional<chainerx::Array>(biases[i]) : absl::nullopt;
                outputs[i] = Call(inputs[i], weights[i], sub_bias, stride, pad, 1, out_size, out_dtype, absl::nullopt);
            }
            return chainerx::Concatenate(outputs, 1);
        } else {
