...
class TestNStepLstm(op_utils.ChainerOpTest):
    ...
    def generate_inputs(self):
        ...
        c_shape = (self.n_layers, self.batches[0], self.hidden_size)
        h_dtype = self.in_dtypes[0]
        c_dtype = self.in_dtypes[1]
        ws_dtype = self.in_dtypes[2]
        bs_dtype = self.in_dtypes[3]
        ip_dtype = self.in_dtypes[4]
        
        h = array_utils.uniform(h_shape, h_dtype)
        c = h.copy()#array_utils.uniform(c_shape, c_dtype)
        in_size = self.input_size
        out_size = self.hidden_size
        xs = [array_utils.uniform((self.batches[b], in_size), ws_dtype) for b in range(len(self.batches))]
        ...
        for n in range(self.n_layers):
            for i in range(8):
                inputs.append(array_utils.uniform((out_size, w_in(n, i)), ws_dtype) )
            for i in range(8):
    ...
    def forward_chainer(self, inputs):
        h, c, ws, bs, xs =  self.process_input(inputs)
        out = chainer.functions.n_step_lstm(self.n_layers, 0.0, h, c, ws, bs, xs)
        rets = []
        rets.append(out[0])
        rets.append(out[1])
