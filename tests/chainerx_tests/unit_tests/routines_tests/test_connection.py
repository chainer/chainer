...
def _create_conv_args(
        xp, device, x_shape, w_shape, b_shape, stride, pad, dilate, cover_all,
        float_dtype):
    ...
    if device.backend.name == 'cuda':  # cover_all is not supported by CUDA.
        cover_all = False
    return x, w, b, stride, pad, dilate, cover_all
...
class _ConvTestBase(object):
    ...
    def forward_chainerx(self, inputs):
        if len(inputs) == 2:
            ...
            x, w, b = inputs
        y = chainerx.conv(
            x, w, b, self.stride, self.pad, self.dilate, self.cover_all)
        return y,
        ...
        chainer.testing.from_pytest_parameterize(
            'x_shape,w_shape,b_shape,stride,pad,dilate', [
                ...
                ((2, 3, 2, 6, 3), (2, 3, 1, 3, 2), None,
                 (1, 2, 3), (2, 0, 1), 1),
                ((1, 3, 4), (5, 3, 2), None, 3, 2, 2),
                ((2, 3, 4, 10), (2, 3, 3, 3), None, 2, (2, 0), (1, 2)),
            ]),
        chainer.testing.from_pytest_parameterize(
...
class _ConvTransposeTestBase(object):
    ...
    def setup(self):
        ...
        if cover_all is None:
            ...
            dilate_tup = (dilate,) * ndim if \
                isinstance(dilate, int) else dilate
            outsize = tuple(
                chainer.utils.conv.get_deconv_outsize(
                    d, k, s, p, cover_all, dil)
                for (d, k, s, p, dil)
                in zip(in_dims, kernel_size, stride_tup, pad_tup, dilate_tup))
    ...
    def forward_chainerx(self, inputs):
        ...
        y = chainerx.conv_transpose(
            x, w, b, self.stride, self.pad, self.dilate, self.outsize)
        return y,
    ...
    def forward_chainer(self, inputs):
        ...
        y = chainer.functions.deconvolution_nd(
            x, w, b, self.stride, self.pad, self.outsize, self.dilate)
        y = F.cast(y, self.out_dtype)
        return y,
        ...
        chainer.testing.from_pytest_parameterize(
            'x_shape,w_shape,b_shape,stride,pad,dilate', [
                ...
                ((2, 3, 5, 6, 3), (3, 2, 1, 3, 2), None,
                 (1, 2, 3), (2, 0, 1), 1),
                ((2, 3, 10, 10), (3, 2, 3, 3), None, 2, (2, 0), 2),
                ((2, 3, 5, 6, 3), (3, 2, 1, 3, 2), None,
                 (1, 2, 3), (2, 0, 1), (3, 2, 1)),
            ]),
        chainer.testing.from_pytest_parameterize(
