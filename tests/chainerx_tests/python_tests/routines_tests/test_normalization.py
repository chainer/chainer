...
def test_batch_norm(device, x_shape, reduced_shape, eps, decay, axis, float_dtype):
    ...
    x_chx, gamma_chx, beta_chx, running_mean_chx, running_var_chx = create_args(chainerx)
    ...
    y_chx = chainerx.batch_norm(x_chx, gamma_chx, beta_chx, running_mean=running_mean_chx, running_var=running_var_chx, **optional_args)
    ...
    chainerx.testing.assert_allclose_ex(running_var_chx, running_var_np, rtol=1e-6, atol=1e-6)
...
def test_fixed_batch_norm(device, x_shape, reduced_shape, eps, axis, float_dtype):
    ...
    y_chx = chainerx.fixed_batch_norm(x_chx, gamma_chx, beta_chx, mean=mean_chx, var=var_chx, **optional_args)
