...
def test_float_dtypes():
    ...
    assert all(dtype in chainerx.testing.all_dtypes for dtype in chainerx.testing.float_dtypes)
...
def test_signed_dtypes():
    ...
    assert all(dtype in chainerx.testing.all_dtypes for dtype in chainerx.testing.signed_dtypes)
...
@chainerx.testing.parametrize_dtype_specifier('spec', dtypes=['int32', 'float64'])
def test_parametrize_dtype_specifier_with_dtypes(spec):
    assert chainerx.dtype(spec).name in ('int32', 'float64')


@chainerx.testing.parametrize_dtype_specifier('spec', dtypes=[], additional_args=('foo',))
