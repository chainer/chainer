...
def _simple_instance_normalization(x, gamma, beta, xp, eps=1e-5):
    batch_size, channels = x.shape[:2]
    axis = tuple(range(2, x.ndim))
    mean = xp.mean(x, axis=axis, keepdims=True)
    std = xp.sqrt(xp.var(x, axis=axis, keepdims=True) + eps)
    x = (x - mean) / std
...
class TestInstanceNormalization(unittest.TestCase):
    ...
    def check_forward(self, args):
        ...
        def func(*args):
            return functions.instance_normalization(*args)
        ...
        y_simple_gn = _simple_instance_normalization(*self.args, xp)
        testing.assert_allclose(
            y.array, y_simple_gn, **self.check_forward_options)
        ...
        if self.shape[0] > 1:
            ...
            testing.assert_allclose(
                y.array, y_one_each.array, **self.check_forward_options)
    ...
    def check_backward(self, args, y_grad):
        def func(*args_):
            return functions.instance_normalization(*self.args)
