...
class TestLocalConvolution2DFunction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.b = numpy.random.uniform(
            -1, 1, (oh, ow, out_channels,)).astype(self.x_dtype)

        self.x = numpy.random.uniform(
            -1, 1, (2, 3, 4, 4)).astype(self.x_dtype)
    ...
    def check_forward(self, x_data, W_data, b_data):
        ...
        if b_data is not None:
            for i in moves.range(b_data.shape[0]):
                for j in moves.range(b_data.shape[1]):
        ...
        f2 = convolution_2d.Convolution2DFunction(self.stride, 1)
