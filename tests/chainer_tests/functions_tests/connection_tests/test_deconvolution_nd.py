...
class TestDeconvolutionND(testing.FunctionTestCase):
    ...
    def setUp(self):
        ...
        self.check_backward_options = {'atol': 3e-5, 'rtol': 3e-4}
    ...
    def generate_inputs(self):
        ...
        if self.nobias:
            return x, W
        b = numpy.random.uniform(-1, 1, self.out_channels).astype(self.x_dtype)
        return x, W, b
    ...
    def check_forward_consistency(self, backend_config):
        ...
        x_gpu = backend_config.get_array(x)
        W_gpu = backend_config.get_array(W)
        b_gpu = None if b is None else backend_config.get_array(b)
        with backend_config:
