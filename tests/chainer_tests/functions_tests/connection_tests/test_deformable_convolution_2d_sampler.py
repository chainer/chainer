...
class TestDeformableConvolution2DSamplerFunctionZeroOffset(unittest.TestCase):
    ...
    def check_forward(self, x, offset, W, b, stride, pad, use_cudnn=True):
        x = chainer.Variable(x)
        offset = chainer.Variable(offset)
        out = deformable_convolution_2d_sampler(
            x, offset, W, b, stride, pad, use_cudnn=use_cudnn).data
        expeceted = convolution_2d(
            x, W, b, stride, pad, use_cudnn=use_cudnn).data
    ...
    def test_forward_gpu(self):
        self.check_forward(
            ...
            cuda.to_gpu(self.W),
            cuda.to_gpu(self.b),
            self.stride, self.pad, use_cudnn=self.use_cudnn)
...
class TestDeformableConvolution2DSamplerFunctionLeftBottomOffset(
    ...
    def test_forward_cpu(self):
        self.check_forward(
            self.x, self.offset, self.W, self.b, self.stride,
            self.pad, use_cudnn=self.use_cudnn)
