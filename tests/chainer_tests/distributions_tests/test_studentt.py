...
def params_init(shape):
    nu = numpy.random.uniform(1, 10, shape).astype(numpy.float32)
    ...
    params = {"nu": nu, "loc": loc, "scale": scale}
    sp_params = {"df": nu, "loc": loc, "scale": scale}
...
@testing.distribution_unittest(distributions.StudentT, stats.t,
