...
class TestNStepGRU(unittest.TestCase):
    ...
    def check_forward(self, h_data, c_data, xs_data):
        ...
        for batch, seq in enumerate(self.xs):
            for layer in range(self.n_layer):
                ...
                hs = []
                for x in seq:
                    i = sigmoid(x.dot(p.w0.data.T) + h_prev.dot(p.w3.data.T) +
                                p.b0.data + p.b3.data)
                    r = sigmoid(x.dot(p.w1.data.T) + h_prev.dot(p.w4.data.T) +
                                p.b1.data + p.b4.data)
                    h_bar = numpy.tanh(
                        x.dot(p.w2.data.T) + r * (h_prev.dot(p.w5.data.T) + p.b5.data)
                        + p.b2.data)
    ...
    def check_backward(
            self, h_data, xs_data, gh_data, gc_data, gys_data):
