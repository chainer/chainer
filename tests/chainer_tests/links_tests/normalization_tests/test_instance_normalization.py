...
class InstanceNormalizationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        aggr_axes = (0,) + tuple(range(2, ndim + 2))
        shape = (5,) + param_shape + (2,) * ndim
        self.expander = (None, Ellipsis) + (None,) * self.ndim

        options = {'use_gamma': True, 'use_beta': True}
        if hasattr(self, 'axis'):
            options['axis'] = self.axis
        self.link = links.InstanceNormalization(
            param_shape, self.dtype, **options
        )
        ...
        if self.test and self.use_running_statistics:
            ...
            self.var = numpy.random.uniform(
                0.5, 1, param_shape).astype(dtype)
    ...
    def check_forward(self, x_data):
        ...
        testing.assert_allclose(
            y_expected, y.array, **self.check_forward_optionss)
...
class InstanceNormalizationTestWithoutGammaAndBeta(unittest.TestCase):
