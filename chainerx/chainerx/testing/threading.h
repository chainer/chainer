...
#define CHAINERX_TEST_THREAD_SAFE_BASE_CLASS_(test_case_name, parent_class)   \
...
// GTEST_TEST_(test_case_name, test_name, parent_class, ::testing::internal::GetTypeId<parent_class>())
...
#define TEST_THREAD_SAFE(test_case_name, test_name)                                                                        \
    CHAINERX_TEST_THREAD_SAFE_BASE_CLASS_(CHAINERX_TEST_DUMMY_CLASS_NAME_(test_case_name, test_name), ::testing::Test)     \
                                                                                                                           \
    CHAINERX_TEST_(test_case_name, test_name##_SingleThread, CHAINERX_TEST_DUMMY_CLASS_NAME_(test_case_name, test_name)) { \
        RunThreadSafeTestBodyWithLeakDetection(1);                                                                         \
        CHAINERX_ASSERT(is_run_skipped() ^ (run_count() == 1));                                                            \
    }                                                                                                                      \
                                                                                                                           \
    CHAINERX_TEST_(test_case_name, test_name##_MultiThread, CHAINERX_TEST_DUMMY_CLASS_NAME_(test_case_name, test_name)) {  \
...
#define TEST_THREAD_SAFE_F(test_fixture, test_name)                                                                      \
    CHAINERX_TEST_THREAD_SAFE_BASE_CLASS_(CHAINERX_TEST_DUMMY_CLASS_NAME_(test_fixture, test_name), test_fixture)        \
                                                                                                                         \
    CHAINERX_TEST_F_(test_fixture, test_name##_SingleThread, CHAINERX_TEST_DUMMY_CLASS_NAME_(test_fixture, test_name)) { \
        RunThreadSafeTestBodyWithLeakDetection(1);                                                                       \
        CHAINERX_ASSERT(is_run_skipped() ^ (run_count() == 1));                                                          \
    }                                                                                                                    \
                                                                                                                         \
    CHAINERX_TEST_F_(test_fixture, test_name##_MultiThread, CHAINERX_TEST_DUMMY_CLASS_NAME_(test_fixture, test_name)) {  \
        RunThreadSafeTestBodyWithLeakDetection(2);                                                                       \
        CHAINERX_ASSERT(is_run_skipped() ^ (run_count() == 1));                                                          \
    }                                                                                                                    \
                                                                                                                         \
...
#define TEST_THREAD_SAFE_P(test_case_name, test_name)                                                                        \
    CHAINERX_TEST_THREAD_SAFE_BASE_CLASS_(CHAINERX_TEST_DUMMY_CLASS_NAME_(test_case_name, test_name), test_case_name)        \
                                                                                                                             \
