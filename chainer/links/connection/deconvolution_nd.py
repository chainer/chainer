...
class DeconvolutionND(link.Link):
    ...
    Args:
        ...
        initialW (numpy.ndarray or cupy.ndarray): Initial weight array.
            If ``None``, the default initializer is used. May be an
            initializer instance of another value the same with that
            :func:`~chainer.init_weight` function can take.
        initial_bias (numpy.ndarray or cupy.ndarray): Initial bias array.
            If ``None``, the bias vector is set to zero. May be an initializer
            instance of another value the same with that
            :func:`~chainer.init_weight` function can take.
