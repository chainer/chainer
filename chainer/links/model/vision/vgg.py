...
def _retrieve(name, url, model):
    ...
    def create_cache(path):
        sys.stderr.write(
            'Loading caffemodel (it may take few minutes)\n')
        sys.stderr.flush()
        path_caffemodel = download.cached_download(url)
        VGGLayers.convert_caffemodel_to_npz(path_caffemodel, path)
        return npz.load_npz(path, model)

    def load_cache(path):
        return npz.load_npz(path, model)

    return download.cache_or_load_file(path, create_cache, load_cache)
