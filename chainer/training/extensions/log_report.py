...
class LogJsonlReport(extension.Extension):

    """__init__(\
keys=None, trigger=(1, 'epoch'), postprocess=None, filename='log.jsonl')

    Trainer extension to output the accumulated results to a log file.

    This extension accumulates the observations of the trainer to
    :class:`~chainer.DictSummary` at a regular interval specified by a supplied
    trigger, and writes them into a log file in JSON Lines format, which is a
    list of JSON objects delimited by a newline (``LF``). It is suitable for
    stream processing and works well with line-oriented tools in UNIX.

    There are two triggers to handle this extension. One is the trigger to
    invoke this extension, which is used to handle the timing of accumulating
    the results. It is set to ``1, 'iteration'`` by default. The other is the
    trigger to determine when to emit the result. When this trigger returns
    True, this extension appends the summary of accumulated values both to the
    list of past summaries on memory and the log file. Then, this extension
    makes a new fresh summary object which is used until the next time that the
    trigger fires.
        ...
        if self._trigger(trainer):
            ...
            if self._log_name is not None:
                ...
                with open(path, 'a', newline='\n') as f:
                    json.dump(stats_cpu, f, indent=None, separators=(',', ':'))
                    # write an LF as a delimiter
                    f.write('\n')
