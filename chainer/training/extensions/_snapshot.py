...
def _find_snapshot_files(fmt, path):
    ...
    # detect and to properly show errors, just ignored or fails so
    # far.
    prefix = fmt.split('{')[0]
    suffix = fmt.split('}')[-1]
...
class _Snapshot(extension.Extension):
    ...
    def initialize(self, trainer):
        ...
        if hasattr(self.writer, '_add_cleanup_hook') and \
           self.num_retain > 0 and isinstance(self.filename, str):
