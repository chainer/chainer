...
class _CheckBackward(object):
    ...
    def _forward_for_backward_gradients(self):
        ...
        xs = [variable.Variable._init_unchecked(x) for x in x_data]
        y = func(*xs)
        y = _as_tuple(y)
    ...
    def _directional_numeric_gradients(self, directions, y0_data):
        ...
        x_vars = [variable.Variable._init_unchecked(x) for x in x_data]
        variables = (
        ...
        def g():
            ...
            for i in range(len(x_vars)):
                if no_grads[i]:
                    ...
                    g_x_vars.append(variable.Variable._init_unchecked(data))
                    j += 1
            # Parameters
