...
class Convolution2DFunction(function_node.FunctionNode):
    ...
    def check_type_forward(self, in_types):
        ...
        type_check.expect(
            ...
            x_type.ndim == 4,
            w_type.ndim == 4,
            x_type.shape[ic] == w_type.shape[ic] * self.groups,
        )
    ...
    def _get_out_size(self, inputs):
        ...
        h, w = utils.nchw_shape(x.shape, self.tensor_layout)[2:]
    ...
    def forward_gpu(self, inputs):
        ...
        out_c = W.shape[0]
        n = x.shape[0]
        y_shape = utils.my_shape((n, out_c, out_h, out_w), self.tensor_layout)
    ...
    def backward(self, indexes, grad_outputs):
        ...
        ret = []
        if 0 in indexes:
            xh, xw = utils.nchw_shape(x.shape, self.tensor_layout)[2:]
...
class Convolution2DGradW(function_node.FunctionNode):
    ...
    def __init__(self, conv2d):
        ...
        self.kh, self.kw = utils.nchw_shape(W_node.shape,
                                            conv2d.tensor_layout)[2:]
        self.sy = conv2d.sy
        self.sx = conv2d.sx
        self.ph = conv2d.ph
        self.pw = conv2d.pw
        ...
        self.tensor_layout = conv2d.tensor_layout
    ...
    def _forward_cudnn(self, x, gy):
        out_c = utils.nchw_shape(gy.shape, self.tensor_layout)[1]
        c = utils.nchw_shape(x.shape, self.tensor_layout)[1]
        iC = c
        iCg = int(iC / self.groups)
        gW_shape = utils.my_shape((out_c, iCg, self.kh, self.kw),
                                  self.tensor_layout)
        gW = cuda.cupy.empty(gW_shape, dtype=self.W_dtype)
