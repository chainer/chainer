...
class RReLU(function_node.FunctionNode):
    ...
    def forward_gpu(self, x):
        xp = cuda.cupy
        if chainer.config.train:
            self.r = xp.random.uniform(self.lower, self.upper, x[0].shape)
        else:
    ...
    def backward(self, indexes, grad_outputs):
        ...
        return _RReLUGrad(x, y, self.r).apply(grad_putputs)
...
class _RReLUGrad(function_node.FunctionNode):
    ...
    def forward_cpu(self, inputs):
        gy, = inputs
        gy = gy.copy()
        gy *= np.where(gy[0] < 0, self.r, 1)
        return gy,

    def forward_gpu(self, inputs):
         gy, = inputs
         gy = _kern()(self.y, gy, self.slope)
         return gy,
